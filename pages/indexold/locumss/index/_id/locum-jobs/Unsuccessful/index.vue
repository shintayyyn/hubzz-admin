	...
	<div class="absolute top-0 bottom-0 right-0 left-0 flex flex-col">
		<div class="page-overlap flex-1 flex flex-col self-end bg-trout shadow-lg">
            ...
            <div class="mx-6">
                ...
                <div style="w-full">
                    <div class="flex flex-col">
                        ...
                        <!-- BODY -->
                        <nuxt-link
                        v-for="(locumUserUnsuccessfulJob, index) in locumUserUnsuccessfulJobs"
                        :key="`locumUserUnsuccessfulJob-${index}`"
                        :to="`/locums/${locumUser.id}/locum-jobs/view-locum-job/${locumUserUnsuccessfulJob.id}`"
                        class="flex no-underline shadow-lg rounded-lg bg-waterloo hover:bg-waterloo-light mt-2"
                        >
                            <div style="width: 20%;">
                                <div class="flex text-white text-sm p-4">
                                    <span>{{ locumUserUnsuccessfulJob.job_number }}</span>
                                </div>
                            </div>
                            <div style="width: 15%;">
                                <div class="flex text-white text-sm p-4">
                                    <span>{{ locumUserUnsuccessfulJob.platform_job.practice.surgery.name }}</span>
                                </div>
                            </div>
                            <div style="width: 15%;">
                                <div class="flex text-white text-sm p-4">
                                    <span>{{ locumUserUnsuccessfulJob.platform_job.title }}</span>
                                </div>
                            </div>
                            <div style="width: 16%;">
                                <div class="flex text-white text-sm p-4">
                                    <span>{{ locumUserUnsuccessfulJob.platform_job.date_start }}</span>
                                </div>
                            </div>
                            <div style="width: 16%;">
                                <div class="flex text-white text-sm p-4">
                                    <span>{{ locumUserUnsuccessfulJob.platform_job.date_end }}</span>
                                </div>
                            </div>
                            <div style="width: 16%;">
                                <div class="flex text-white text-sm p-4">
                                    <span>{{ locumUserUnsuccessfulJob.platform_job.date_created }}</span>
                                </div>
                            </div>
                        </nuxt-link>
                        <!-- BODY -->
            ...
            <div v-if="locumJobPageCount > 1">
                ...
                <button class="p-2 m-1 rounded-lg border text-sm text-white hover:bg-waterloo-light" @click="goToPage(activePage + 1)">Next</button>																									<!-- ^ Removed the FF. code in this area: v-if="showPage(locumJobPage) TAKE A NOTE OF THIS"-->
            </div>
            <!-- PAGINATION -->
            <nuxt-child/>
		</div>
...
export default{
    ...
    try {
        ...
        response = await app.$axios.get(`api/v1/admin/jobs?locum_detail_id=${locumUser.locum_detail.id}&locum_status=Unsuccessful`,{ params })
        const locumUserUnsuccessfulJobs = response.data.data.jobs

        response = await app.$axios.get(`api/v1/admin/jobs/count?locum_detail_id=${locumUser.locum_detail.id}&locum_status=Unsuccessful`,{ params })
        const itemCount = response.data.data.count

        console.log(locumUserUnsuccessfulJobs)
