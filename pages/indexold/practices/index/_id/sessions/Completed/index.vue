        ...
        <div class="flex flex-col">
           <!-- HEADER -->
            ...
            >
                <div style="width: 20%;">
                    <div class="flex text-white text-sm p-4">
                        <span>{{ practiceCompletedJob.job_number }}</span>
                    </div>
                </div>
                <div style="width: 15%;">
                    <div class="flex text-white text-sm p-4">
                        <span>{{ practiceCompletedJob.platform_job.practice.surgery.name }}</span>
                    </div>
                </div>
                <div style="width: 15%;">
                  <div class="flex text-white text-sm p-4">
...
export default{
  ...
  async asyncData({ app, route }) {
    try {
      ...
      response = await app.$axios.get(`/api/v1/admin/jobs?practice_id=${route.params.id}&status=Completed`,{ params })
      const practiceCompletedJobs = response.data.data.jobs
      
      response = await app.$axios.get(`/api/v1/admin/jobs/count?practice_id${route.params.id}&status=Completed`,{ params })
      const itemCount = response.data.data.count

      console.log(practiceCompletedJobs)
