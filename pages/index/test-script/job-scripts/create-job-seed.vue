    ...
    <div class="flex-1 flex flex-col ">
      ...
      <div class="w-full lg:w-1/4 ">
        ...
        <AppSuggestSelect
          ...
          :label="'Practice'"
        />
        <div v-if="form.practice_id" class="p-2 font-bold">
        ...
        <div class="flex flex-row">
              ...
              <template v-if="form.role">
                <AppFilterSearch
                  ...
                  :professionCategoryId="
                    selectedProfession && selectedProfession.profession_category ? selectedProfession.profession_category.id.toString() : null
                  "
                  ...
                  <AppInput
                    ...
                    @unchecked="
                      form.compliance_document_id.splice(
                        form.compliance_document_id.findIndex(item => item === parseInt($event)),
                        1
                      )
                    "
          ...
          <div class="w-full md:w-1/2 lg:pl-4 mb-4">
            ...
            <div class="rounded-lg shadow-lg px-4 md:px-8 py-4 mt-4">
              ...
              <AppInput v-model="form.description" :type="'textarea'" :name="'description'" :label="'Description'" :resize="false" />
                ...
                <AppInput
                  ...
                  @unchecked="
                    form.session_requirements.splice(
                      form.session_requirements.findIndex(item => item === $event),
                      1
                    )
                  "
              ...
              <AppInput
                ...
                :placeholder="
                  'For e.g. the first 2 hours of the session is for booked appointments, 3rd hour is walk-ins, and home visits to x number of patients to the end of the session'
                "
                :resize="false"
              />
                ...
                <AppInput
                  ...
                  @unchecked="
                    form.mandatory_training_id.splice(
                      form.mandatory_training_id.findIndex(item => item === parseInt($event)),
                      1
                    )
                  "
                ...
                <AppInput
                  ...
                  @unchecked="
                    form.other_mandatory_training_id.splice(
                      form.other_mandatory_training_id.findIndex(item => item === parseInt($event)),
                      1
                    )
                  "
        ...
        <div v-if="jobStatus === 'Applied' || jobStatus === 'Allocated' || jobStatus === 'Ongoing' || jobStatus === 'Completed'">
          ...
          >
            ...
            <template v-slot:status_slot="slotProps">
              <div class="px-4 py-1 rounded-full w-32 text-center mx-auto my-1" :class="statusStyle(slotProps.item.status)">
                {{ slotProps.item.status }}
              </div>
            </template>
            <template v-slot:compliance_slot="slotProps">
              <div class="px-4 py-1 rounded-full w-32 text-center mx-auto my-1" :class="complianceStatusStyle(slotProps.item.compliance_status)">
      ...
      <transition name="fade">
        <div v-if="toPublish" class="job-notification-modal p-4 rounded font-bold ">
          <p class="text-center pb-2 mb-4 border-b-2 border-gray-600 text-lg font-bold">
  ...
  computed: {
    ...
    selectedProfessionComplianceCategory() {
      ...
      const professionComplianceCategory = this.professionComplianceCategories.find(
        professionComplianceCategory => professionComplianceCategory.id === profession.profession_compliance_category_id
      )
    ...
    compliances() {
      ...
      const complianceDocuments = this.professionComplianceCategories.reduce((compliances, professionComplianceCategory) => {
        const {
        ...
        if (professionComplianceCategory.id === this.selectedProfessionComplianceCategory.id) {
          [referenceComplianceDocuments, mandatoryComplianceDocuments, optionalComplianceDocuments].forEach(complianceDocuments => {
      ...
      return this.practiceProfessionComplianceCategoryComplianceDocuments
        .filter(practiceProfessionComplianceCategoryComplianceDocument => {
          ...
          return (
            professionComplianceCategoryId === this.selectedProfessionComplianceCategory.id && complianceDocumentIds.includes(complianceDocumentId)
          )
        })
    ...
    practice_rate() {
      ...
      const practiceRate = practiceRates.find(item => item.type === profession.profession_category_name)
  ...
  methods: {
    ...
    tailorForPractice() {
      ...
      Promise.all([
        ...
        this.$axios.get('/api/v1/professions').then(response => response.data.data.professions),
        this.$axios.get(`/api/v1/admin/practices/${this.form.practice_id}`).then(response => response.data.data.practice),
        this.$axios.get('/api/v1/profession-compliance-categories').then(response => {
        ...
        .then(responses => {
          ...
          const { report_to: reportTo, email, extra_information: extraInformation } = profileProfile
    ...
    getSchedule(schedule, total_gross_locum_wages, total_working_hours, deductions, total_lates, hasError, job_parts) {
      this.form.schedules = []
      ...
      schedule.forEach((sched, index) => {
        if (sched.shifts && sched.shifts.length) {
          let dateErrIndex = this.shiftErrors.findIndex(err => err.field === `shift-${sched.date}`)
          ...
          sched.shifts.forEach((shift, i) => {
            ...
            if (shift.time_start) {
              let startIndex = this.shiftErrors.findIndex(err => err.field === `time_start-s${index}-${i}`)
            ...
            if (shift.time_end) {
              let endIndex = this.shiftErrors.findIndex(err => err.field === `time_end-s${index}-${i}`)
            ...
            if (shift.locum_detail_rate_type_id !== 0 && shift.locum_detail_rate_type_id !== '') {
              let rateTypeIndex = this.shiftErrors.findIndex(err => err.field === `locum_detail_rate_type_id-s${index}-${i}`)
            ...
            if (shift.shift_id !== 0 && shift.shift_id !== '') {
              let shiftIdIndex = this.shiftErrors.findIndex(err => err.field === `shift_id-s${index}-${i}`)
            ...
            if (shift.rate !== 0 && shift.rate !== '') {
              let rateIndex = this.shiftErrors.findIndex(err => err.field === `rate-s${index}-${i}`)
    ...
    canPublish() {
      ...
      if (!this.shiftErrors.length) {
        ...
        this.form.clinical_system_id = this.form.clinical_system.map(item => item.value)
        ...
        if (['false', false].includes(this.auto_assign_job)) {
          if (['true', true].includes(this.selection_notification)) {
            this.form.selection_date = `${this.$moment(this.selection_date.date, 'YYYY-MM-DD').format('YYYY-MM-DD')} ${this.selection_date.time}`
          }
        }

        this.form.favorite_only_until = null
        if (['true', true].includes(this.bank_first)) {
          this.form.favorite_only_until = `${this.$moment(this.favorite_only_until.date, 'YYYY-MM-DD').format('YYYY-MM-DD')} ${
        ...
        this.form.ir35 = this.selectedProfession && this.selectedProfession.profession_category.name === 'GP' ? this.form.ir35 : false
        ...
        this.$axios
          ...
          .then(() => {
            if (!this.shiftErrors.length && !this.hasShiftError && !this.formError.length && !has_conflict) {
              this.toPublish = true
            }
            this.loading = false
          })
              ...
              if (err.response.data.error_messages && err.response.data.error_messages.length > 0) {
                ...
                if (sched_has_conflict) {
                  ...
                  sched_has_conflict.conflictSchedules.forEach(item => {
                    this.shiftErrors.push({
                      field: `conflict-${this.$moment(item.date, 'YYYY-MM-DD').format('DD/MM/YYYY')}-${item.index}`,
                      message: 'This schedule has a conflict with another schedule.'
                    })
                  })
                  let conflictDates = sched_has_conflict.conflictSchedules.map(item => item.date).filter(item => item)
                  let job_parts = []
                  conflictDates.forEach(date => {
                    let job_part = this.job_parts.find(item => item.dates.includes(date))
                  ...
                  this.$store.commit('SET_NOTIFICATION', {
                    enabled: true,
                    status: 'danger',
                    text: [`Conflict schedule on Job Part/s (${job_parts.join(',')})`],
                    duration: 3000
                  })
                }
                ...
                } else if (this.formError.map(err => ['schedules', 'dates'].includes(err.field)).includes(true)) {
      ...
      if (this.shiftErrors.length) {
        let has_empty_sched_dates = this.shiftErrors.filter(err => err.field.includes('shift-'))
        let job_parts = []
        if (has_empty_sched_dates.length) {
          has_empty_sched_dates.forEach(err => {
            let empty_date = err.field.split('-')[1]
            let job_part = this.job_parts.find(part => part.dates.includes(this.$moment(empty_date, 'DD/MM/YYYY').format('YYYY-MM-DD')))
    ...
    async createJob() {
      ...
      } else if (['true', true].includes(this.selection_notification) && this.selection_date.date && this.selection_date.time) {
        notRequired.push('selection_date')
      }
      ...
      } else if (['true', true].includes(this.bank_first) && this.favorite_only_until.date && this.favorite_only_until.time) {
        notRequired.push('favorite_only_until')
      }
