      ...
      <div class="py-4">
        ...
        <div class="py-4">
          <div class="mx-2 md:mx-4  flex flex-wrap items-center">
            ...
            <div class="px-2 flex">
              <AppButton label="Reset" class="mx-1" :inClass="'mt-2 bg-gray-800 hover:bg-gray-900 '" :background="''" @click="resetOverrideCreated" />

              <AppButton label="Save" class="mx-1" :inClass="'mt-2 bg-gray-800 hover:bg-gray-900 '" :background="''" @click="saveOverrideCreated" />
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      locumUserJobsOrderBy: ['locum_user_applied_at_in_gb_formatted:desc'],
  ...
  methods: {
    setValues() {
      this.overrideCreatedDate =
        this.locumUser && this.locumUser.override_created_at
      ...
      this.overrideLastJobApplicationDateDate =
        this.locumUser && this.locumUser.override_last_job_application_date
    ...
    saveOverrideCreated() {
      ...
      this.loading = true
      this.$axios
        .put(`/api/v1/admin/locum-users/${this.$route.params.locumUserId}/override-created-at`, data)
        .then(response => {
    ...
    saveOverrideLastJobApplicationDate() {
      ...
      this.loading = true
      this.$axios
        .put(`/api/v1/admin/locum-users/${this.$route.params.locumUserId}/override-last-job-application-date`, data)
        .then(response => {
          this.locumUser = response.data.data.user
    ...
    getLocumUser() {
      return Promise.all([
        this.$axios.get(`/api/v1/admin/locum-users/${this.$route.params.locumUserId}`).then(response => response.data.data.user)
      ]).then(responses => {
        const [locumUser] = responses
    ...
    getLocumUserAppliedInJobs() {
      ...
      return Promise.all([
        this.$axios
          .get('/api/v1/admin/test/jobs/count', {
            params: {
              ...filters
            }
          })
        ...
        .then(responses => {
          const [locumUserJobsCount, locumUserJobs] = responses
