			...
			>
				<div v-if="!editing" class="relative w-full overflow-hidden text-gray-300 text-sm px-2 md:p-2">
					<button
						...
						@click="editRate"
					>{{ practice && practice.rates && practice.rates.length > 0 ? "Edit" : "Add" }}</button>

					<div class="flex py-1">GP Rate (Per Hour)</div>
					<div
						class="text-white text-lg font-semibold mx-3 mb-2 leading-tight focus:outline-none"
					>{{ gpRate }}</div>
					<div class="flex py-1">Others Rate (Per Hour)</div>
					<div
				...
				>
					<div class="flex items-center justify-between py-1">GP Rate (Per Hour)</div>
					...
					<div class="flex items-center justify-between py-1">Others Rate (Per Hour)</div>
					<input
	...
	methods: {
		gpRateKeyPressHandler(e) {
			if (e.key < 10 || e.key === "Backspace") {
				if (
					e.key < 10 &&
					this.toPutPracticeRate.gp_rate.toString().includes(".")
				) {
					const decimal = this.toPutPracticeRate.gp_rate
						.toString()
						.split(".")[1];
					...
					if (
						...
						e.target.selectionStart >
							this.toPutPracticeRate.gp_rate.toString().length - 3
					) {
						e.preventDefault();
					}
				}
		...
		othersRateKeyPressHandler(e) {
			if (e.key < 10 || e.key === "Backspace") {
				if (
					e.key < 10 &&
					this.toPutPracticeRate.others_rate.toString().includes(".")
				) {
					const decimal = this.toPutPracticeRate.others_rate
						.toString()
						.split(".")[1];
					...
					if (
						...
						e.target.selectionStart === e.target.selectionEnd &&
						e.target.selectionStart >
							this.toPutPracticeRate.others_rate.toString().length - 3
					) {
						e.preventDefault();
					}
				}
