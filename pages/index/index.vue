<template>
  <div class="flex-1 flex flex-col overflow-hidden py-2 px-4 md:px-8">
    ...
    <div class="text-sm font-hairline text-white">
      Work in progress
    </div>
    ...
    <div v-if="promptModal" class="shield" @click="closeModals()" />
	...
	async created (){
		...
		if(this.authAdminPermissions.includes("View Locum Compliance Detail")) {
			await this.$axios.$get(
				`/api/v1/admin/locum-compliance-documents`
			).then(res => {
				locumDocAlert = res.data.locum_compliance_documents
			})
