  ...
  <div>
    <div class="page-overlap flex-1 flex flex-col self-end">
      ...
      <div class="flex-col justify-start items-center w-full border p-3 rounded-lg flex my-2">
        ...
        <div class="flex flex-col md:flex-row w-full items-center">
          <div class="md:px-1 w-full lg:w-1/4 md:w-1/3">
            <AppDate v-model="registeredDateStart" label="Date Registered Start" format="YYYY-MM-DD" />
          </div>

          <div class="md:px-1 w-full lg:w-1/4 md:w-1/3">
            <AppDate v-model="registeredDateEnd" label="Date Registered End" format="YYYY-MM-DD" />
          </div>
        </div>
      ...
      <div v-if="canGenerateReports" class="flex-wrap justify-start items-center w-full p-3 flex my-2">
        <div class="md:px-1 flex flex-wrap w-full justify-end">
          <button
...
export default {
  ...
  computed: {
    canGenerateReports() {
      return this.$store.getters['permissions'].includes('Generate Reports')
    },

    filterParams() {
      return {
        locum_name_includes: this.locumNameIncludes ? this.locumNameIncludes : undefined,
        not_approved_compliance_document_names_includes: this.notApprovedComplianceDocumentNamesIncludes
          ? this.notApprovedComplianceDocumentNamesIncludes
          : undefined,
        not_upload_compliance_document_names_includes: this.notUploadComplianceDocumentNamesIncludes
    ...
    isDownloadDisabled() {
      return this.downloading || this.locumUploadedDocuments.length === 0
    },

    downloadButtonClass() {
      return this.locumUploadedDocuments.length === 0 ? 'bg-gray-500' : 'bg-sunglow hover:bg-sunglow-dark'
    },

    itemCountInfo() {
      const firstItem = Math.min(this.limit * this.activePage - this.limit + 1, this.count)
      const lastItem = Math.min(
        this.limit * this.activePage - this.limit + (this.loading ? this.limit : this.locumUploadedDocuments.length),
        this.count
      )
  ...
  mounted() {
    const {
      ...
      not_approved_compliance_document_names_includes: notApprovedComplianceDocumentNamesIncludes,
      not_upload_compliance_document_names_includes: notUploadComplianceDocumentNamesIncludes,
    ...
    this.notApprovedComplianceDocumentNamesIncludes = notApprovedComplianceDocumentNamesIncludes ? notApprovedComplianceDocumentNamesIncludes : ''
    this.notUploadComplianceDocumentNamesIncludes = notUploadComplianceDocumentNamesIncludes ? notUploadComplianceDocumentNamesIncludes : ''
  ...
  methods: {
    formatOrderBy(orderBy) {
      let replaced = ''
      if (orderBy.length > 0) {
        replaced = orderBy[0].replace(/_/g, ' ')
    ...
    getLocumUploadedDocuments() {
      ...
      Promise.all([
        this.$axios
          .get('/api/v1/admin/reports/locum-uploaded-documents/count', {
            params: {
              ...this.filterParams
            }
          })
        ...
        this.$axios
          .get('/api/v1/admin/reports/locum-uploaded-documents', {
            params: {
              ...this.filterParams,
              order_by: this.orderBy,
              limit: this.limit,
        ...
        this.$axios
          .post(
            '/api/v1/admin/reports/locum-uploaded-documents/generate-key',
            {
        ...
        .then(results => {
          const [count, locumUploadedDocuments, downloadToken] = results
