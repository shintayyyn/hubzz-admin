    ...
    <div class="page-overlap flex-1 flex flex-col self-end">
      ...
      <div class="flex-col justify-start items-center w-full border p-3 rounded-lg flex  my-2">
        <div class="flex flex-col md:flex-row w-full items-center">
          ...
          <div class="md:px-1 w-full lg:w-1/4 md:w-1/3">
            <AppInput v-model="minCompliancePercentage" placeholder="0.00" type="number" label="Min % Compliant" />
          </div>

          <div class="md:px-1 w-full lg:w-1/4 md:w-1/3">
            <AppInput v-model="maxCompliancePercentage" placeholder="100.00" type="number" label="Max % Compliant" />
          </div>
        </div>
        ...
        <div class="flex flex-col md:flex-row w-full items-center">
          ...
          <div class="md:px-1 w-full lg:w-1/4 md:w-1/3">
            <AppDate v-model="approvedDateStart" label="Date of Approved Start" format="YYYY-MM-DD" />
          </div>

          <div class="md:px-1 w-full lg:w-1/4 md:w-1/3">
            <AppDate v-model="approvedDateEnd" label="Date of Approved End" format="YYYY-MM-DD" />
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    canGenerateReports() {
      return this.authAdminPermissions.includes('Generate Reports')
    },
    ...
    isDownloadDisabled() {
      return this.downloading || this.locumCompletedCompliances.length === 0
    },

    downloadButtonClass() {
      return this.locumCompletedCompliances.length === 0 ? 'bg-gray-500' : 'bg-sunglow hover:bg-sunglow-dark'
    },

    itemCountInfo() {
      const firstItem = Math.min(this.limit * this.activePage - this.limit + 1, this.count)
      const lastItem = Math.min(
        this.limit * this.activePage - this.limit + (this.loading ? this.limit : this.locumCompletedCompliances.length),
        this.count
      )
  ...
  methods: {
    ...
    getLocumCompletedCompliances() {
      ...
      Promise.all([
        this.$axios
          .get('/api/v1/admin/reports/locum-completed-compliances/count', {
            params: {
              ...this.filterParams
            }
          })
        ...
        this.$axios
          .get('/api/v1/admin/reports/locum-completed-compliances', {
            params: {
              ...this.filterParams,
              order_by: this.orderBy,
              limit: this.limit,
        ...
        this.$axios
          .post(
            '/api/v1/admin/reports/locum-completed-compliances/generate-key',
            {
        ...
        .then(results => {
          const [count, locumCompletedCompliances, downloadToken] = results
