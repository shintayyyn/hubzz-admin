            ...
            <div class="mx-6">
                ...
                <div style="w-full">
                    <div class="flex flex-col">
                        ...
                        <!-- BODY -->
                        <nuxt-link
                        v-for="(locumUserDeclinedJob, index) in locumUserDeclinedJobs"
                        :key="`locumUserDeclinedJob-${index}`"
                        :to="`/locums/${locumUser.id}/locum-jobs/view-locum-job/${locumUserDeclinedJob.id}`"
                        class="flex no-underline shadow-lg rounded-lg bg-waterloo hover:bg-waterloo-light mt-2"
                        >
                            <div style="width: 20%;">
                                <div class="flex text-white text-sm p-4">
                                    <span>{{ locumUserDeclinedJob.job_number }}</span>
                                </div>
                            </div>
                            <div style="width: 15%;">
                                <div class="flex text-white text-sm p-4">
                                    <span>{{ locumUserDeclinedJob.platform_job.practice.surgery.name }}</span>
                                </div>
                            </div>
                            <div style="width: 15%;">
                                <div class="flex text-white text-sm p-4">
                                    <span>{{ locumUserDeclinedJob.platform_job.title }}</span>
                                </div>
                            </div>
                            <div style="width: 16%;">
                                <div class="flex text-white text-sm p-4">
                                    <span>{{ locumUserDeclinedJob.platform_job.date_start }}</span>
                                </div>
                            </div>
                            <div style="width: 16%;">
                                <div class="flex text-white text-sm p-4">
                                    <span>{{ locumUserDeclinedJob.platform_job.date_end }}</span>
                                </div>
                            </div>
                            <div style="width: 16%;">
                                <div class="flex text-white text-sm p-4">
                                    <span>{{ locumUserDeclinedJob.platform_job.date_created }}</span>
                                </div>
                            </div>
                        </nuxt-link>
                        <!-- BODY -->
...
export default{
    ...
    try {
        ...
        response = await app.$axios.get(`api/v1/admin/jobs?locum_detail_id=${locumUser.locum_detail.id}&locum_status=Declined`,{ params })
        const locumUserDeclinedJobs = response.data.data.jobs

        response = await app.$axios.get(`api/v1/admin/jobs/count?locum_detail_id=${locumUser.locum_detail.id}&locum_status=Declined`,{ params })
        const itemCount = response.data.data.count

        console.log(locumUserDeclinedJobs)
