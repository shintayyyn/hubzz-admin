    ...
    <div class="page-overlap flex-1 flex flex-col self-end">
      ...
      <div class="flex-wrap justify-start items-center w-full border p-3 rounded-lg flex  my-2">
        <div class="md:px-1 w-full lg:w-1/4 md:w-1/3">
          <AppInput v-model="nameIncludes" placeholder="Search by Name" type="text" label="Full Name" />
        </div>
...
export default {
  ...
  mounted() {
    const { name_includes: nameIncludes, domain: domain, order_by: orderBy = [], page } = this.$route.query
  ...
  methods: {
    ...
    getBogusRegistrations() {
      ...
      Promise.all([
        this.$axios
          .get('/api/v1/admin/reports/bogus-registrations/count', {
            params
          })
          .then(responses => {
        ...
        this.$axios
          .get(`/api/v1/admin/reports/bogus-registrations`, {
            params: {
              ...params,
              order_by: this.orderBy,
              limit: this.limit,
        ...
        this.$axios
          .post(
            '/api/v1/admin/reports/bogus-registrations/generate-key',
            {
        ...
        .then(results => {
          const [count, bogusRegistrations, downloadToken] = results
