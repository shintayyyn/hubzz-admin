      ...
      <div class="flex-wrap justify-start items-center w-full border p-3 rounded-lg flex my-2">
        <div class="flex flex-row w-full ">
          <div class=" w-full lg:w-1/3 md:w-1/3">
            ...
            <div>
              <AppMonthYear v-model="monthYear" :isBefore="true" />
            </div>
          </div>
        </div>
    ...
    columnDetails() {
      return [
        ...
        },
        {
          ...
          sort_key: null,
          column: item =>
            item.ave_days_spent_resolve_disputes !== null
              ? `${item.ave_days_spent_resolve_disputes > 1 ? item.ave_days_spent_resolve_disputes.toFixed(2) : 'Less Than 1'} Day(s)`
              : 'N/A',
          justify: 'start',
          flexGrow: 1,
          flexShrink: 0
  ...
  methods: {
    ...
    getPracticeDisputes() {
      ...
      const params = {
        month: this.$moment(this.monthYear).format('MMMM'),
        year: this.$moment(this.monthYear).format('YYYY'),
        last_date_of_month: this.$moment(this.monthYear)
          .endOf('month')
          .format('YYYY-MM-DD')
      }
      Promise.all([
        this.$axios
          .get('/api/v1/admin/reports/practice-disputes/count', {
            params
          })
          .then(responses => {
        ...
        this.$axios
          .get('/api/v1/admin/reports/practice-disputes', {
            params: {
              ...params,
              order_by: this.orderBy,
              limit: this.limit,
        ...
        this.$axios
          .post(
            '/api/v1/admin/reports/practice-disputes/generate-key',
            {
        ...
        .then(results => {
          const [count, practiceDisputes, downloadToken] = results
