...
const API_BASE = '/api/v1/admin/reports/registered-practices'
...
export default {
  ...
  computed: {
    ...
    itemCountInfo() {
      ...
      const lastItem = Math.min(this.offset + (this.loading ? this.limit : this.registeredPractices.length), this.count)
    ...
    getPractices() {
      ...
      Promise.all([
        ...
        this.$axios
          ...
          .then(res => res.data.data.registered_practices),

        this.$axios
          .post(`${API_BASE}/generate-key`, { filename: 'registeredPractices.csv' }, { params: { ...params, order_by: this.orderBy } })
          .then(res => res.data.data.token)
      ])
        .then(([count, registeredPractices, downloadToken]) => {
          this.count = count
