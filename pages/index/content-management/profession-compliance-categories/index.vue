<template>
  <div class="flex-1 flex flex-col py-2 px-4 md:px-6">
    ...
    <ContentManagementTable
      :limit="limit"
      :items="professionComplianceCategories"
      :getItemKey="(item) => item.id"
      :getItemLink="(item) => `/profession-compliance-categories/${item.id}`"
    ...
    <nuxt-link
      v-if="
        $route.name !== 'index-content-management-profession-compliance-categories'
          && $route.name !== 'index-content-management-profession-compliance-categories-index'
      "
      class="bg-shield z-511 fixed inset-0 opacity-50"
      to="/profession-compliance-categories"
    />
...
<script>
  ...
  export default {
      ...
      columnDetails () {
        return [
          ...
          },
          {
            ...
            sort_key: 'profession_count',
            column: (item) => item.profession_count,
            justify: 'end',
            flexGrow: 0,
            flexShrink: 0,
          },
    ...
    methods: {
      getProfessionComplianceCategories () {
        this.loading = true
        this.professionComplianceCategories = []
        Promise.all([
          this.$axios.get('/api/v1/admin/profession-compliance-categories/count').then((responses) => {
            return responses.data.data.count
          }),
          this.$axios.get('/api/v1/admin/profession-compliance-categories', {
            params: {
            ...
            return responses.data.data.profession_compliance_categories
          }),
