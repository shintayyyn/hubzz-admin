<template>
  <div class="flex-1 flex flex-col py-2 px-4 md:px-6">
    ...
    <div v-if="false" class="flex justify-end">
      <nuxt-link
        to="/content-management/practice-users/create"
        class="
    ...
    <ContentManagementTable
      ...
      :getItemLink="(item) => true ? null : `/content-management/practice-users/${item.id}`"
    ...
    <nuxt-link
      v-if="$route.name !== 'index-content-management-practice-users'"
      class="bg-shield z-511 fixed inset-0 opacity-50"
      to="/content-management/practice-users"
    />
  
    <nuxt-child
      @practiceUserCreated="practiceUserCreatedHandler"
      @practiceUserUpdated="practiceUserUpdatedHandler"
      @practiceUserDeleted="practiceUserDeletedHandler"
    />
  </div>
</template>
  ...
  export default {
    ...
    computed: {
      itemCountInfo () {
        ...
        const lastItem = Math.min(this.limit * this.activePage - this.limit + this.practiceUsers.length, this.count)
      ...
      offset () {
        return Math.max(this.activePage * this.limit - this.limit + this.practiceUsers.length, 0)
      },
    ...
    methods: {
      ...
      loadMorePracticeUsers (limit) {
        ...
        this.$axios.get('/api/v1/admin/practice-user-payloads', {
          ...
          responses.data.data.practiceUsers.forEach((practiceUser) => {
            this.practiceUsers.push(practiceUser)
          })
        }).catch((err) => {
          console.log('err', err)
        })
      ...
      practiceUserCreatedHandler (data) {
        const {
          practiceUser
        } = data
        ...
        if (this.search === '') {
          ...
          if (this.activePage === this.pages && this.practiceUsers.length < this.limit) {
            this.practiceUsers.push(practiceUser)
          }
        }
      },

      practiceUserUpdatedHandler (data) {
        const {
          practiceUser
        } = data

        const index = this.practiceUsers.findIndex(({ id }) => id === practiceUser.id)

        if (index > -1) {
          this.practiceUsers.splice(index, 1, practiceUser)
        }
      },

      practiceUserDeletedHandler (data) {
        const {
          id,
        } = data
        ...
        if (this.activePage > this.pages) {
          ...
          const index = this.practiceUsers.findIndex((practiceUser) => practiceUser.id === id)

          if (index > -1) {
            this.practiceUsers.splice(index, 1)
