<template>
  <div ref="modalContainer">
    <div class="p-4 md:p-6">
      <div class="flex flex-row items-start justify-between flex-wrap">
        <div class="flex items-center justify-between w-full leading-tight">
          ...
          <div class="flex py-1 text-sm md:text-base">
            <div
              v-if="editingPermissions == false && authAdminPermissions.includes('Edit Role')"
            ...
            <div
              v-if="editingPermissions == true && authAdminPermissions.includes('Edit Role')"
              class="flex items-center mr-1 md:mr-2 py-1 px-2 md:px-4 md:py-2 rounded-lg bg-yellow-500 hover:bg-yellow-400 cursor-pointer text-black"
      ...
      <div v-if="editingPermissions == false" class="my-2 md:my-4">
        <div class="masonry-container three overflow-hidden">
          <div v-if="rolePermissions.find(permission => permission.category === 'Locum')" class="item pb-3 md:pt-1">
          ...
          <div v-if="rolePermissions.find(permission => permission.category === 'Practice')" class="item pb-3 md:pt-1">
            <div class="text-lg font-semibold">
              Practice Management
            </div>
            <div
          ...
          <div v-if="rolePermissions.find(permission => permission.category === 'Billing')" class="item pb-3 md:pt-1">
            <div class="text-lg font-semibold">
          ...
          <div v-if="rolePermissions.find(permission => permission.category === 'Miscellaneous')" class="item pb-3 md:pt-1">
            <div class="text-lg font-semibold">
              Miscellaneous
            </div>
            <div
              ...
              :class="permission.name.includes('View') ? 'text-sm mx-2' : 'text-sm mx-6'"
            >
              {{ permission.category === 'Miscellaneous' ? permission.name : null }}
            </div>
          </div>
          <div v-if="rolePermissions.find(permission => permission.category === 'User')" class="item pb-3 md:pt-1">
            <div class="text-lg font-semibold">
              User Management
            </div>
            <div
          ...
          <div v-if="rolePermissions.find(permission => permission.category === 'Change Email Requests')" class="item pb-3 md:pt-1">
            <div class="text-lg font-semibold">
              Change Email Requests Management
            </div>
            <div
              v-for="(permission, index) in rolePermissions"
              :key="`permission-${index}`"
          ...
          <div v-if="rolePermissions.find(permission => permission.category === 'Reports')" class="item pb-3 md:pt-1">
            <div class="text-lg font-semibold">
              Reports Management
            </div>
            <div
          ...
          <div v-if="rolePermissions.find(permission => permission.category === 'Standard Terms')" class="item pb-3 md:pt-1">
            <div class="text-lg font-semibold">
          ...
          <div v-if="rolePermissions.find(permission => permission.category === 'Referral Lottery')" class="item pb-3 md:pt-1">
            <div class="text-lg font-semibold">
              Referral Lottery Management
            </div>
            <div
              v-for="(permission, index) in rolePermissions"
              :key="`permission-${index}`"
      ...
      <div v-if="editingPermissions == true" class="flex flex-col mt-4">
        <AppInput
          v-model="form.name"
          :type="'text'"
          :name="'name'"
        ...
        <p class="text-xs sm:text-sm py-1 pr-2 font-bold">
          ...
          <span v-if="formError.find(item => item.field === 'permission_id')" class="mx-2 bg-red-300 text-red-700 py-1 px-2 text-xs">{{
        ...
        <div class="masonry-container three">
          <div v-for="(permissionGroup, permissionGroupIndex) in permissions" :key="permissionGroupIndex" class="item p-2">
            <div class="flex flex-col">
              <div class="w-full flex flex-row items-center pb-1">
                <input
                  :id="permissionGroup.category"
                  type="checkbox"
                  :checked="isChecked(permissionGroup.permissions, permissionGroup.category)"
                  @change="checkAll(permissionGroupIndex, $event.target.checked)"
                />

                <label class="font-bold md:text-xl pl-1 leading-none flex items-center" :for="permissionGroup.category">{{ permissionGroup.category }} Management</label>
              </div>

              <div v-for="(hierarchyPermissionGroup, hierarchyPermissionGroupIndex) in hierarchyPermissions" :key="hierarchyPermissionGroupIndex">
                <!-- {{item.category}}
									{{item.permissions}}-->
                <template v-if="permissionGroup.category === hierarchyPermissionGroup.category">
                  <div class="w-full p-2">
                    <div class="flex flex-col w-full">
                      <div class="pl-4 w-full">
                        <div v-for="(permission, permissionIndex) in hierarchyPermissionGroup.permissions" :key="permission.id" class="flex flex-col px-1 w-full">
                          <input
                            :id="permission.id"
                            v-model="permission.done"
                            type="checkbox"
...
export default {
    ...
    try {
      ...
      await app.$axios.$get(`/api/v1/admin/admin-permissions`).then(res => {
        res.data.permissions.forEach(permission => {
          let hasPermission = rolePermissions.find(item => item.id === permission.id)
          if (hasPermission) {
      ...
      allPermissions.forEach(permission => {
        if (categories.length === 0) {
          ...
          let hasSameCategory = categories.find(item => item.category === permission.category)
          if (!hasSameCategory) {
            categories.push({
              category: permission.category,
              permissions: []
      ...
      allPermissions.forEach(permission => {
        let foundCategory = categories.find(item => item.category === permission.category)
  ...
  methods: {
    getRoles() {
      this.$axios.$get(`/api/v1/admin/admin-roles/${this.$route.params.roleId}`).then(res => {
    ...
    getPermissions() {
      this.$axios.$get(`/api/v1/admin/admin-permissions`).then(res => {
        res.data.permissions.forEach(permission => {
          let hasPermission = this.role.permissions.find(item => item.id === permission.id)
          if (hasPermission) {
      ...
      permission.forEach(item => {
        item.permissions.forEach(item => {
          if (subCategories.length === 0) {
            ...
            let foundCategory = subCategories.find(categ => categ.subcategory === item.subcategory)
            if (!foundCategory) {
              subCategories.push({
                category: item.category,
                subcategory: item.subcategory,
    ...
    async setHierarchy(subCategories) {
      ...
      await this.permissions.forEach(item => {
        item.permissions.forEach(permission => {
          let findSub = subCategories.find(sub => sub.category === permission.category && sub.subcategory === permission.subcategory)
