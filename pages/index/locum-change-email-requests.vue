<template>
  <div class="flex-1 flex flex-col py-2 px-2 md:px-6 overflow-y-auto">
    ...
    <div class="mt-2">
      <div class="flex justify-start overflow-x-auto">
        <nuxt-link
          ...
          :class="true ? 'border rounded-lg text-black border-sunglow bg-sunglow' : ''"
        >
          <span>Locum</span>
        </nuxt-link>
        <nuxt-link
          ...
          style="font-family:Nunito"
          class="mr-5 p-3 text-sm font-bold cursor-pointer whitespace-no-wrap"
          :class="false ? 'border rounded-lg text-black border-sunglow bg-sunglow' : ''"
        >
          <span>Practice</span>
        </nuxt-link>
      </div>
      ...
      <div class="flex py-2">
        <div class="relative">
          <input
            v-model="search"
            style="width: 250px;"
            class="rounded-lg border-2 border-transparent text-sm  p-2 pr-6 focus:border-sunglow focus:outline-none bg-waterloo"
          ...
          <button v-if="search" class="absolute top-0 right-0 bottom-0 mr-3 md:mr-1" @click=";(search = ''), searchSubmit()">
            <svgicon name="times-solid" height="12" width="12" class=" hover:text-yellow-500 fill-current -mx-2 md:-mx-6" />
          </button>
        </div>
      </div>

      <div class="flex flex-col w-full justify-end">
        <div class="md:w-full relative flex flex-col md:flex-row justify-end md:items-center md:items-end md:py-2 py-0">
    ...
    <div v-if="count === 0" class="mt-2 w-full text-center ">
      <span>No locum change email requests.</span>
    </div>

    <nuxt-link v-if="false" class="locum-shield" :to="{ name: 'index-locum-change-email-requests' }" />
...
export default {
  ...
  methods: {
    ...
    getLocumChangeEmailRequests() {
      this.loading
      Promise.all([
        this.$axios.get('/api/v1/admin/locum-change-email-requests/count').then(response => response.data.data.count),
        this.$axios.get('/api/v1/admin/locum-change-email-requests').then(response => response.data.data.locum_change_email_requests)
      ])
        .then(responses => {
          console.log('responses', responses)
          const [count, locumChangeEmailRequests] = responses
