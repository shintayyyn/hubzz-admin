<template>
  <div class="items-center text-sm text-white">
    <div v-if="practiceInvoice.paid_at_in_gb_formatted" class="mx-4 my-2 font-semibold text-lg">
      {{ '* This invoice has been marked as paid on ' + practiceInvoice.paid_at_in_gb_formatted }}
    </div>
    <div v-if="practiceInvoice.unpaid_at_in_gb_formatted" class="mx-4 my-2 font-semibold text-lg">
...
export default {
  ...
  async asyncData({ app, route }) {
    try {
      ...
      response = await app.$axios.$get(`/api/v1/admin/practices/${practiceInvoice.practice_id}`)
