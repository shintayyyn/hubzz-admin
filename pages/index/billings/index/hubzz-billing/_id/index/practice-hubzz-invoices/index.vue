  ...
  <div class="w-full overflow-hidden">
    ...
    >
      ...
      <template v-slot:period="slotProps">
        <div>
          {{ $moment(slotProps.item.date_start).format('DD/MM/YYYY') + ' - ' + $moment(slotProps.item.date_end).format('DD/MM/YYYY') }}
        </div>
      </template>
      ...
      <template v-slot:issuedAt="slotProps">
        <div>
          {{
            $moment(slotProps.item.date_created_in_gb).isSame($moment(), 'day')
              ? slotProps.item.date_created_in_gb_formatted_relative
              : $moment(slotProps.item.date_created_in_gb).format('DD/MM/YYYY')
          }}
        </div>
      </template>
    ...
    <div v-if="showPaidModal == true" class="shield" @click="closeModals()" />
  </div>
</template>
<script>
...
export default {
  ...
  async asyncData({ app, route, store }) {
    try {
      ...
      let { page = 1, order_by = ['date_created:desc'] } = route.query
