<template>
  <section class="mt-2">
    <template v-if="shouldShowHeaderTabs">
        ...
        <div class="flex flex-wrap">
          <div class="w-1/3">
            ...
            <p class="flex flex-wrap items-center  text-sm p-2 font-semibold">
              ...
              <span class="py-2 px-4 text-sm rounded-lg font-extrabold" :class="practiceTypeStyle(practice.type)">{{ practice.type }}</span>
              <span
      ...
      <div class="flex flex-row justify-start overflow-x-auto border-b border-gray-500 mb-4 pt-1">
        <nuxt-link
          ...
          :class="isHubzzInvoicesTabActive ? 'border-b-4 border-gray-500' : 'text-gray-600'"
        >
          HUBZZ Invoices
        </nuxt-link>
        <nuxt-link
          ...
          class="p-3 text-sm font-bold cursor-pointer whitespace-no-wrap mx-1"
          :class="isLocumInvoicesTabActive ? 'border-b-4 border-gray-500' : 'text-gray-600'"
        >
          Invoices by Locums
        </nuxt-link>
      </div>
...
<script>
const HUBZZ_INVOICE_ROUTE_NAMES = [
  'index-billings-index-hubzz-billing-id-index-practice-hubzz-invoices',
  'index-billings-index-hubzz-billing-id-index-practice-hubzz-invoices-hubzzInvoiceId'
]

const LOCUM_INVOICE_ROUTE_NAMES = [
  'index-billings-index-hubzz-billing-id-index-invoices-by-locums',
  'index-billings-index-hubzz-billing-id-index-invoices-by-locums-locumInvoiceId'
]
...
export default {
  ...
  computed: {
    shouldShowHeaderTabs() {
      return [...HUBZZ_INVOICE_ROUTE_NAMES, ...LOCUM_INVOICE_ROUTE_NAMES].includes(this.$route.name)
    },
    isHubzzInvoicesTabActive() {
      return HUBZZ_INVOICE_ROUTE_NAMES.includes(this.$route.name)
    },
    isLocumInvoicesTabActive() {
      return LOCUM_INVOICE_ROUTE_NAMES.includes(this.$route.name)
    },
    getRoute() {
      return tab => {
        if (!tab) {
        ...
        delete query.status
        return {
          path: tab ? `/billings/hubzz-billing/${this.$route.params.id}/${tab}` : `/billings/hubzz-billing/${this.$route.params.id}`,
          query
        }
      }
    }
