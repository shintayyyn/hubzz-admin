<template>
    <div>
      ...
      <div class="flex flex-wrap overflow-hidden md:px-4">
        ...
        <div class="flex-col text-white w-full m-2 pb-3 border-b border-sunglow text-sm md:text-base">
          <div class="flex my-1">
            <span class="w-24 pr-3">Date Sent:</span>
            <span class="font-bold">{{$moment(email.sender.created_at).format('MMM DD, YYYY | HH:MM:ss') }}</span>
          </div>
          <div class="flex my-1">
            <span class="w-24 pr-3">From: </span>
            <span class="font-bold">{{email.sender.email}}</span>
          </div>
          <div class="flex my-1">
            <div class="w-24 pr-3">Domain:</div>
            <div class="font-bold">{{email.sender.domain}}</div>
          </div>
        </div>
       
      </div>
      <div class="my-2 md:mx-6">
        <div class="flex flex-col bg-waterloo text-white py-2 px-4 w-full h-full">
          {{email.message}}
        </div>
      </div>
    </div>
