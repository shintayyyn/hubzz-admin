    ...
    <div class="overflow-x-auto overflow-y-hidden">
      ...
      <div v-if="unsuccessfulJobs.length === 0">
        ...
        <div
          ...
          style="font-family: Nunito"
        >
          This locum hasn't been rejected to any jobs yet.
        </div>
      </div>
      <div v-else>
        <AppJobHeaderSort
          :locumUser="user"
          :locumTabStatus="'Unsuccessful'"
        ...
        <div class="w-full overflow-x-auto">
          ...
          <div
            ...
            @click="$router.push(`/locums/${$route.params.id}/locum-jobs/locum-unsuccessful-jobs/${item.id}`)"
          >
  ...
  export default {
    ...
    async created () {
      ...
      Promise.all([
        ...
        this.$axios.$get(`/api/v1/admin/jobs/count`, { params }).then(res => {
          this.$store.commit(
            "jobs/SET_LOCUM_UNSUCCESSFUL_JOBS_COUNT",
            res.data.count
          )
          this.perPage = 10
    ...
    methods: {
      async getJobPartsPromiseAll () {
        ...
        this.$store.commit("jobs/SET_LOCUM_UNSUCCESSFUL_JOBS_COUNT", responseCount.data.count)
      ...
      getUnsuccessfulJobs (orderBy) {
        let offset =
          this.perPage * (parseInt(this.$route.query.unsuccessful_job_page) - 1)
        let params = {
          viewing_locum_user_id: this.$route.params.id,
          locum_status: "Unsuccessful",
