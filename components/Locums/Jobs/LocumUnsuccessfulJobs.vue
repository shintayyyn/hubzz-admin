        ...
        <div v-else>
          <div class="table border-separate overflow-x-auto" style="border-spacing: 0 10px;"> 
            ...
            <nuxt-link 
              ...
              :to="{ path: `/locums/${user.id}/locum-jobs/locum-unsuccessful-jobs/${item.id}`}"
              :key="`item-${index}`" 
    ...
    methods:{
      getUnsuccessfulJobs(orderBy){
        ...
        offset = this.perPage * (parseInt(this.$route.query.unsuccessful_job_page) - 1)
          this.$axios.$get(`/api/v1/admin/jobs?locum_detail_id=${this.user.locum_detail.id}&locum_status=Unsuccessful&order_by=${orderBy}&order_by=id%3Adesc&limit=${this.perPage}&offset=${offset}`).then(res=>{
