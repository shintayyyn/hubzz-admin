    ...
    <div class="overflow-x-auto overflow-y-hidden">
      ...
      <div v-if="appliedJobs.length === 0">
        ...
        <div
          ...
          style="font-family: Nunito"
        >
          This locum has not applied to any jobs yet.
        </div>
      </div>
      <div v-else>
        ...
        <div class="w-full overflow-x-auto">
          ...
          <div
            ...
            @click="$router.push(`/locums/${$route.params.id}/locum-jobs/locum-applied-jobs/${item.id}`)"
          >
  ...
  export default {
    ...
    async created () {
      ...
      Promise.all([
        this.$axios.$get(`/api/v1/admin/jobs/count`, { params }).then(res => {
          this.$store.commit(
            "jobs/SET_LOCUM_APPLIED_JOBS_COUNT",
            res.data.count
          )
          this.perPage = 10
    ...
    methods: {
      async getJobPartsPromiseAll () {
        ...
        this.$store.commit("jobs/SET_LOCUM_APPLIED_JOBS_COUNT", responseCount.data.count)
      ...
      getAppliedJobs (orderBy) {
        ...
        this.$axios.$get(`/api/v1/admin/jobs`, { params }).then(res => {
          this.$store.commit("jobs/SET_LOCUM_APPLIED_JOBS", res.data.jobs)
          this.$store.commit("jobs/TOGGLE_LOADING", false)
        })
      },
