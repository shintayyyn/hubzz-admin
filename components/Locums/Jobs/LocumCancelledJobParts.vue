          ...
          <div class="w-full overflow-x-auto" > 
            ...
            >
              ...
              <div class="flex flex-col md:justify-center sm:w-1/2 md:w-1/6 px-1 xl:px-2 py-2 align-middle md:text-center">
                ...
                <span class="break-words"><span class="">{{ $moment(item.date_created, 'YYYY-MM-DD[T]HH:mm:ss.SSS[Z]').format('DD/MM/YYYY, h:mm:ss a') }}</span></span>
              </div>
            </div>
          </div>
        </div>
...
export default {
    ...
    methods:{
      getCancelledJobs(orderBy){
        let offset = this.perPage * (parseInt(this.$route.query.cancelled_job_page) - 1)
        let params = {
          viewing_locum_user_id : this.user.id,
          locum_status : 'Cancelled',
