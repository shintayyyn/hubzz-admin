    ...
    <div class="overflow-x-auto overflow-y-hidden">
      ...
      <div v-if="locumWithdrawnJobParts.length == 0">
        ...
        <div
          ...
          style="font-family: Nunito"
        >
          This locum has no withdrawn jobs.
        </div>
      </div>
      <div v-else>
        <AppJobHeaderSort
        ...
        <div class="w-full overflow-x-auto">
          ...
          <div
            ...
            @click="$router.push(`/locums/${$route.params.id}/locum-jobs/locum-withdrawn-jobs/${item.id}`)"
          >
            ...
            <div class="flex flex-col md:justify-center sm:w-1/2 md:w-1/6 px-1 xl:px-2 py-2 align-middle md:text-center">
              ...
              <span>{{ item.declined_at_in_gb_formatted }}</span>
            </div>
          </div>
        </div>
      </div>
  ...
  export default {
    ...
    async created () {
      ...
      Promise.all([
        ...
        this.$axios
          ...
          .then(res => {
            this.$store.commit(
              "jobs/SET_LOCUM_WITHDRAWN_JOBS_COUNT",
              res.data.count
            )
            console.log("res", res)
    ...
    methods: {
      async getJobPartsPromiseAll () {
        ...
        this.$store.commit("jobs/SET_LOCUM_WITHDRAWN_JOBS_COUNT", responseCount.data.count)
