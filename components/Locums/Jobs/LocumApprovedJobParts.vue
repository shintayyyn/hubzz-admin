<template>
  <div>
    <div class="overflow-x-auto overflow-y-hidden">
      ...
      <div v-else>
        <AppJobHeaderSort :locumUser="user" :locumTabStatus="'Approved'" :currentPage="currentPage" :isJobParts="true" />
      ...
      <div v-if="!locumApprovedJobParts.length == 0" class="m-10 xl:-ml-32">
        <AppPagination
          :total="total"
  ...
  async created() {
    ...
    Promise.all([
      ...
      this.$axios.$get(`/api/v1/admin/job-parts/count`,{ params }).then(res => {
        this.$store.commit('jobs/SET_LOCUM_APPROVED_JOBS_COUNT', res.data.count)
        console.log('res',res)
