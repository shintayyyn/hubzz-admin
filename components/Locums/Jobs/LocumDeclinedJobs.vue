<template>
    <div>
      <div class="overflow-x-auto overflow-y-hidden">
        ...
        <div v-else>
         <AppJobHeaderSort :locumUser="user" :locumTabStatus="'Declined'" :currentPage="currentPage" />
          <div class="w-full overflow-x-auto" > 
            ...
            <div 
              ...
              @click="$router.push(`/locums/${user.id}/locum-jobs/locum-declined-jobs/${item.id}`)"
              :key="`item-${index}`" 
        ...
        <div v-if="!declinedJobs.length == 0" class="">
          <AppPagination
            :total="total"
...
export default {
    ...
    methods:{
      getDeclinedJobs(orderBy){
        let offset = this.perPage * (parseInt(this.$route.query.declined_job_page) - 1)
        let params = {
          viewing_locum_user_id : this.user.id,
          locum_status : 'Declined',
