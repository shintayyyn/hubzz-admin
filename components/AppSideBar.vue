      ...
      <div v-for="(navigationTab, index) in navigationTabs" :key="index" class="text-sm relative">
        ...
        <nuxt-link :to="navigationTab.route">
          ...
          >
            ...
            <span
              v-if="navigationTab.name === 'Locums' && unseenLocumNotificationsCount > 0"
              class="text-white rounded-sm p-1 bg-red-700 text-xs font-bold flex items-center justify-center"
            ...
            <span
              v-if="navigationTab.name === 'Practices' && unseenPracticeNotificationsCount > 0"
              class="text-white rounded-sm p-1 bg-red-700 text-xs font-bold flex items-center justify-center"
              :style="unseenPracticeNotificationsCount > 99 ? 'font-size: 10px' : ''"
              style="min-width: 20px; height: 20px;"
            >
            ...
            <span
              v-if="navigationTab.name === 'Change Email Requests' && pendingChangeEmailRequestIds.length > 0"
              class="text-white rounded-sm p-1 bg-red-700 text-xs font-bold flex items-center justify-center"
              style="min-width: 20px; height: 20px;"
            >
...
export default {
  ...
  computed: {
    ...
    navigationTabs() {
      ...
      if (
        this.authAdminPermissions.includes('View Locums') ||
      ...
      if (this.authAdminPermissions.includes('View Hubzz Invoices') || this.authAdminPermissions.includes('View Reports')) {
        addedLists.push({
          name: 'Billing',
      ...
      if (this.authAdminPermissions.includes('View Admin Accounts') || this.authAdminPermissions.includes('View Roles')) {
        addedLists.push({
  ...
  async created() {
    if (this.$auth.loggedIn) {
      this.$axios
        .$get(`/api/v1/admin/supports/count`, {
          params: {
            acknowledged: false
          }
        })
  ...
  methods: {
    adminNotificationChangeEmailRequestPendingHandler(changeEmailRequest) {
      const { id: changeEmailRequestId } = changeEmailRequest
