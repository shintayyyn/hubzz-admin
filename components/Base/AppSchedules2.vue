  ...
  watch: {
    schedule_dates(value) {
      ...
      if (value.length) {
        ...
        if (!removedAnItem) {
          let isExisting = this.schedules.find(item => item.date === value[value.length - 1])
          if (!isExisting) {
            this.schedules.push({
  ...
  methods: {
    emitSchedule() {
      if (this.type === 'invoice') {
        ...
        // hasError,
        // hasChanges
        let deduction = this.getJobGrossRate(this.schedules) - this.getJobGrossRate(this.schedules, true)
        this.$emit(
          'getSchedule',
          this.schedules,
