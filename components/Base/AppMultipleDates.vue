<template>
  <div v-on-clickaway="toggledOff" class="flex flex-col py-2 mb-4 md:mb-6 leading-normal">
    ...
    <transition name="fade">
      <div v-if="!isDisplay ? (isOpen ? isOpen : modal) : true" class="relative md:static flex" :class="isDisplay ? 'z-0' : 'z-10'">
        <div class="border bg-transparent" :class="isDisplay ? 'w-full' : 'rounded-b-lg calendar absolute shadow-md'">
...
export default {
    ...
    placeholder() {
      ...
      display =
        this.dates && this.dates.length
          ? dates.length > 1
            ? `${minDate.format('DD/MM/YYYY')} - ${maxDate.format('DD/MM/YYYY')} - (${dates.length} day${dates.length > 1 ? 's' : ''})`
  ...
  created() {
    if (this.value && this.value.length) {
      ...
      let last = this.value.find((item, index) => index + 1 === this.value.length)
      this.lastDate = last
    }
  ...
  methods: {
    ...
    showHover(fullDate) {
      if (this.overlayData) {
        let details = this.overlayData.filter(item => item.date === this.$moment(fullDate).format('YYYY-MM-DD'))
      ...
      if (!this.isAfter) {
        if (this.limitYear) {
          for (let i = 1; i <= this.limitYear; i++) {
            this.yearLists.push(
              this.$moment()
                .subtract(this.maxYearBefore ? this.maxYearBefore - 1 : 0, 'years')
                .subtract(i, 'years')
                .format('YYYY')
            )
          }
    ...
    select(date) {
      if (!this.disableSelection) {
        if (!this.isDisabled(date)) {
          if (!this.dates.includes(this.$moment(date, 'MM-DD-YYYY').format(this.format))) {
            this.dates.push(this.$moment(date, 'MM-DD-YYYY').format(this.format))
          } else {
            let dateIndex = this.dates.findIndex(item => item === this.$moment(date, 'MM-DD-YYYY').format(this.format))
            if (dateIndex > -1) {
