    ...
    <div class="flex items-center">
      ...
      <transition name="fade">
        <div v-if="filterModal" class="flex items-center">
          <AppButton :label="'Clear'" class="mx-1" :class="'border  hover:bg-gray-700 hover:border-none'" @click="clearFilters" />
          <AppButton :label="'Search'" class="mx-1" :class="'border  hover:bg-gray-700 hover:border-none'" @click="filterJobParts" />
        </div>
      </transition>
    </div>
    <transition name="drop-down">
      <div v-if="filterModal" class="flex">
        <div class="md:px-1 w-full lg:w-1/4 md:w-1/3">
          <AppInput v-model="job_number" class="" :type="'text'" :name="'job_number'" :label="'Job number'" />
        </div>
        <div class="md:px-1 w-full lg:w-1/4 md:w-1/3">
          <AppInput v-model="job_title" class="" :type="'text'" :name="'job_title'" :label="'Job title'" />
        </div>
      </div>
    </transition>
    <div v-if="items.length === 0 && loading === false" class="mt-2">
      <div v-if="isFiltered" class="mt-10  w-full text-center" style="font-family: Nunito">
        Job Not Found.
      </div>
      <div v-else-if="!isFiltered" class="mt-10  w-full text-center" style="font-family: Nunito">
    ...
    items() {
      ...
      switch (status) {
        ...
        case 'Unsuccessful':
          ...
          break
        case 'Pending':
          records = this.$store.state.jobs.practice_pending_sessions
          break
        case 'Available':
