<template>
  <div>
    <div class="choose-job-parts-modal shadow-lg">
          ...
          >
            <template v-slot:checker="slotProps">
              <input
                ...
                :disabled="isRowDisabled(slotProps.item)"
              />
              <label :for="slotProps.item" :class="isRowDisabled(slotProps.item) ? 'cursor-not-allowed opacity-50' : ''" />
            </template>

            <template v-slot:invoice_status_slot="slotProps">
              <div class="rounded-full text-center px-4 py-1 w-32" :class="statusStyle(slotProps.item.invoice_status)">
            ...
            <template v-slot:status_slot="slotProps">
              <div class="rounded-full text-center px-4 py-1 w-32" :class="statusStyle(slotProps.item.status)">
                <!-- {{ slotProps.item.status && slotProps.item.status === "Invoiced" ? "Approved" : slotProps.item.status }} -->
                {{ slotProps.item.status }}
              </div>
            </template>
          </AppTableNew>
          <AppButton :label="'Confirm'" :disabled="chosenJobParts.length < 1 ? true : false" @click="emitChosenJobParts()" />
          <div class="text-white">
            <div>Chosen Job Parts / Invoices Count: {{ chosenJobParts.length }}</div>
          </div>
        </template>
        <template v-if="jobPartCount === 0">
      ...
      <div v-if="$route.name.includes('approvedJobPartId')" class="issue-hubzz-invoice-shield" @click="close()" />
    </div>
  </div>
</template>
<script>
