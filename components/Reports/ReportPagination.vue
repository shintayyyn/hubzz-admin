<template>
  <div class="flex justify-end items-center">
    ...
    <button v-for="page in pages" :key="`page_${page}`" :class="activePage === page ? 'text-white' : ''" class="border rounded-lg p-2 m-1 h-full" v-if="showPage(page)" @click="$emit('setPage', page)">
      <span>{{ page }}</span>
    </button>
