    ...
    <div class="overflow-x-auto overflow-y-hidden">
      ...
      <div v-if="ongoingJobParts.length === 0">
        ...
        <div v-else-if="!isFiltered" class="mt-10 w-full text-center text-white">
          This practice has no ongoing sessions.
        </div>
      </div>
      <div v-else>
        <AppJobHeaderSort
      ...
      <div v-if="modal" class="job-shield" @close="modal = false" />
	...
	methods: {
		async getJobPartsPromiseAll () {
          ...
          practice_surgery_id: this.$route.name.includes("practice-surgeries")
			...
			this.$store.commit("jobs/SET_PRACTICE_ONGOING_SESSIONS_COUNT", responseCount.data.count)
		...
		checkRoute (itemId) {
			if (this.$route.name.includes("practice-surgeries")) {
				return {
					path: `/practices/${this.$route.params.id}/practice-surgeries/${this.$route.params.practiceSurgeryId}/surgery-sessions/surgery-ongoing-sessions/${itemId}`
				}
			} else if (this.$route.name.includes("practice-sessions")) {
				return {
					path: `/practices/${this.$route.params.id}/practice-sessions/practice-ongoing-sessions/${itemId}`
				}
			}
		},
