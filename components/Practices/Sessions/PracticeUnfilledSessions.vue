    ...
    <div class="overflow-x-auto overflow-y-hidden">
      ...
      <div v-if="unfilledSessions.length == 0">
        ...
        <div v-else-if="!isFiltered" class="mt-10 w-full text-center text-white">
          This practice has no unfilled session/s.
        </div>
      </div>
      <div v-else>
        <AppJobHeaderSort
...
export default {
	...
	async created () {
		...
		Promise.all([
			this.$axios.$get(`/api/v1/admin/jobs/count`, { params }).then(res => {
				this.$store.commit(
					"jobs/SET_PRACTICE_UNFILLED_SESSIONS_COUNT",
					res.data.count
				)
				this.perPage = 10
	...
	methods: {
		async getJobPartsPromiseAll () {
			...
			this.$store.commit("jobs/SET_PRACTICE_UNFILLED_SESSIONS_COUNT", responseCount.data.count)
			...
			this.$store.commit("jobs/SET_PRACTICE_UNFILLED_SESSIONS", response.data.jobs)
		...
		async getUnfilledSessions (orderBy) {
			...
			await this.$axios
				...
				.then(res => {
					this.$store.commit(
						"jobs/SET_PRACTICE_UNFILLED_SESSIONS",
						res.data.jobs
					)
					this.$store.commit("jobs/TOGGLE_LOADING", false)
				})
