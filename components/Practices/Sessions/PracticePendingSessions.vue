	...
	<div>
		<div class="overflow-x-auto xl:overflow-hidden">
			<div v-if="pendingJobs.length === 0">
				<div class="mt-10 w-full text-center text-white">This practice has no pending session/s.</div>
			</div>
			<div v-else>
				<AppJobHeaderSort
					:practice="practice"
	...
	created() {
		...
		Promise.all([
			this.$axios.$get(`/api/v1/admin/jobs/count`, { params }).then(res => {
				...
				this.$store.commit(
					"jobs/SET_PRACTICE_PENDING_SESSIONS_COUNT",
					res.data.count
				);
				this.perPage = 10;
			...
			.then(() => {
				this.getPendingJobs("date_created:desc"), console.log(this.pendingJobs);
			})
			.catch(err => {
	...
	methods: {
		checkRoute(itemId) {
			if (this.$route.name.includes("practice-surgeries")) {
				return {
					path: `/practices/${this.practice.id}/practice-surgeries/${this.practiceSurgery.id}/surgery-sessions/surgery-pending-sessions/${itemId}`
				};
			} else if (this.$route.name.includes("practice-sessions")) {
				return {
					path: `/practices/${this.practice.id}/practice-sessions/practice-pending-sessions/${itemId}`
				};
			}
		},
		async getPendingJobs(orderBy) {
			...
			await this.$axios
				...
				.then(res => {
					this.$store.commit(
						"jobs/SET_PRACTICE_PENDING_SESSIONS",
						res.data.jobs
					);
