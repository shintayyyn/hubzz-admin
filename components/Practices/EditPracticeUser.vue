    ...
    <div class="flex sm:p-2 max-w-lg">
      ...
      <div v-if="tab1" class="flex flex-col relative p-4 border rounded-lg text-sm w-full">
        ...
        <div v-if="editProfile" class="w-full overflow-hidden  text-sm p-2">
          ...
          <AppInput v-model="toPutPracticeUser.title" :type="'text'" :name="'title'" :label="'Title'" :placeholder="'Mr. / Mrs. / Dr. / etc...'" />
          ...
          <p class="p-2" :class="practiceUser.email && '-light rounded'">
            {{
              practiceUser.email_verified_at
                ? $moment(practiceUser.email_verified_at, 'YYYY-MM-DD[T]HH:mm:ss.SSS[Z]').format('DD/MM/YYYY | HH:mm')
                : 'Not yet verified'
            }}
          </p>
...
export default {
  ...
  watch: {
    ...
    'toChangePassword.password'(value) {
      if (value && value.length < 6) {
        ...
        let index = this.formError.findIndex(item => item.message === 'Password Must Be Atleast 6 Characters')
        let error = this.formError.filter(item => item.message === 'Password Must Be Atleast 6 Characters')
        if (index >= 0) {
  ...
  methods: {
    ...
    async updatePracticeUser(practiceUserId, toPutPracticeUser) {
      try {
        await this.$axios.put(`/api/v1/admin/practice-user-accounts/${practiceUserId}`, toPutPracticeUser)
        const response = await this.$axios.get(`/api/v1/admin/practice-users/${practiceUserId}`)
        const updatedUser = response.data.data.user
        this.toPutPracticeUser = { ...toPutPracticeUser }
        this.$emit('setPracticeUser', updatedUser)
    ...
    deletePracticeUser() {
      ...
      this.$axios
        .put(`/api/v1/admin/practice-users/${this.$route.params.pracUserId}/delete`)
        .then(response => {
