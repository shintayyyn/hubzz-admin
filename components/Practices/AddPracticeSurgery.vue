			...
			<div class="w-full overflow-y-auto px-2">
				<div>
					...
					<template v-if="!practice">
						...
						<p class="text-gray-500 py-2 -mx-2" v-if="!surgeries.length">No Surgeries Found</p>
                  ...
                  [registeredPractice.includes(surgery.id)
					...
					<template v-if="practice && practice.type == 'Hub'">
						...
						<p class="text-gray-500 py-2 -mx-2" v-if="!practiceSpokes.length">No Spoke Found</p>
						<transition-group name="slide" tag="p">
							<div
			...
			await this.$axios
				...
				.then(res => {
					...
					res.data.practices.forEach(spoke => {
						invited = this.practiceSurgeries.find(
