<template>
  <div class="flex-1 flex flex-col overflow-auto">
    ...
    <div class="xl:max-w-full lg:max-w-sm">
      ...
      >
        <template v-slot:updated_at="slotProps">
          <div>
            {{
              slotProps.item.created_at_in_gb_formatted === slotProps.item.updated_at_in_gb_formatted
                ? '(none)'
                : slotProps.item.updated_at_in_gb_formatted
            }}
          </div>
        </template>
    ...
    <div v-if="modal == true" class="role-shield" @click="modal ? (modal = false) : $router.push('/user-management/roles-and-permissions')" />
  </div>
</template>
<script>
