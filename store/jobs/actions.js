...
export default{
    async initializeAdminJobTransactionListener ({ state, commit }) {
    ...
    async fetchLocumJobs({ commit }, payload) {
      ...
      await jobsApi.fetchLocumJobs(this.$axios, payload)
    ...
    async fetchSpecificJob (_, payload) {
      return jobsApi.fetchSpecificJob(this.$axios, payload)
    }
}
