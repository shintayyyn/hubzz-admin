...
export default{
    async initializeAdminJobTransactionListener({ state, commit, dispatch }) {
        ...
        this.$socket.on("locumAppliedToJob",async jobId => {
            let locumJobToTrx = {
                locumUserId : state.viewing_jobs_locum_user_id,
                jobId,
            }
            const response = await jobsApi.fetchSpecificJob(this.$axios,locumJobToTrx)
        ...
        this.$socket.on("practiceCreateNewJob", async jobId => {
            let practiceSessionTrx = {
                practiceId : state.viewing_sessions_practice_id,
                jobId,
            }
            const response = await jobsApi.fetchSpecificJob(this.$axios,practiceSessionTrx)
            const locumJob = response.data.job
            commit('ADD_PRACTICE_AVAILABLE_SESSIONS', jobId)
            commit('ADD_LOCUM_AVAILABLE_JOBS', jobId)
            commit('ADD_LOCUM_MATCHED_JOBS', jobId)
        })
